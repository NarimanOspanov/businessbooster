...
async function findFlats(q, limit) {
  ...
  // объявлении. Порядок: город — обязателен всем; дом опознаём по ЖК, если он
  // назван, иначе по координатам; дальше этаж, комнаты, санузел — их
  // сравниваем, только когда поле известно с обеих сторон, а не заполнено —
  // не исключаем из-за пустоты. Опознание дома (ЖК/координаты) — исключение:
  // оно обязательно, не заполнено — квартира не считается найденной, иначе
  // район и площадь сами по себе сходятся у десятков домов в городе.
  ...
  const r = await pool.request()
    ...
    .query(`
      ...
      WHERE f.area BETWEEN @lo AND @hi
        ...
        -- Дом должен быть подтверждён — ЖК в приоритете, координаты как
        -- запасной путь, когда ЖК не указан. Не «любой из двух», а именно по
        -- порядку: если агент назвал ЖК, сверяем по нему (и он обязан
        -- совпасть — не просто не мешать), а к координатам обращаемся, только
        -- если ЖК вообще неизвестен. Улицу как самостоятельное подтверждение
        -- убрали: имя улицы называет улицу, а не дом, на ней таких домов может
        -- быть десяток. Если не известно ни ЖК, ни координаты — подтвердить
        -- дом нечем, и квартира не считается найденной.
        AND (
          (@complex IS NOT NULL AND f.complex = @complex)
          OR (@complex IS NULL AND @lat IS NOT NULL AND f.lat IS NOT NULL
