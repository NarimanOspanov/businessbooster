...
async function waReply(clinic, text, history) {
  ...
  const system = agentTemplate.buildPromptFor(profile) + live + "\n\n" +
    ...
    "что подтвердим.\n" +
    "Не используй разметку Markdown: в WhatsApp жирный — это одна звёздочка " +
    "по краям слова, а две показываются собеседнику как звёздочки. Проще всего " +
    "писать вообще без выделений.\n" +
