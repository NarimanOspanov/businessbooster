    ...
    if (urlPath === "/api/krisha/objphone" || urlPath === "/api/krisha/objqueue" ||
      ...
      (async () => {
        if (req.method === "GET" || req.method === "HEAD") {
          ...
          // По умолчанию очередь идёт ступенями: продажа квартир, домов,
          // коммерции в Астане и Алматы (Астана первой), затем аренда тех же
          // типов там же, затем то же по остальным городам. Каждый параметр —
          // список через запятую в порядке приоритета; any — без фильтра.
          const filt = (name, def) => {
            const v = (parsed.searchParams.get(name) || "").trim().toLowerCase();
            if (!v) return def;
            return v === "any" || v === "all" ? null : v;
          };
          const dealF = filt("deal", "sale,rent");
          const propF = filt("prop", "flat,house,commercial");
