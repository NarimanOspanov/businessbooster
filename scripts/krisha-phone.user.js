...
(function () {
  ...
  // дальше. retryFn — та же функция (next/autoNext), чтобы повтор шёл по
  // тому же сценарию, что и первая попытка.
  function nextFailed(retryFn) {
    ...
    if (a) a.onclick = function (e) { e.preventDefault(); retryFn(); };
  }
  ...
  function next() {
    nextItem()
      ...
      .catch(function () { nextFailed(next); });
  }
  ...
  function autoNext() {
    nextItem()
      ...
      .catch(function () { nextFailed(autoNext); });
  }
