...
# --- плашка «для кого» ---
tag = "ДЛЯ СТОМАТОЛОГИЙ И МЕДЦЕНТРОВ"
...
POINTS = [
    ...
    ("Отвечает как обученный консультант",
     "По вашему прайсу и расписанию. Чего не знает — не выдумывает"),
    ("Записывает в вашу программу",
     "Смотрит свободное время. Нет программы — заявка приходит в Telegram"),
...
d.text((M + 44, y + 34), "9 000 ₸", font=bold(58), fill=INK)
d.text((M + 44, y + 104), "в месяц — аренда номера,", font=body(28), fill=DIM)
d.text((M + 44, y + 140), "кабинет и поддержка", font=body(28), fill=DIM)

plus_x = M + 330
d.text((plus_x, y + 44), "+", font=bold(52), fill=TEAL)

d.text((plus_x + 76, y + 34), "150 ₸", font=bold(58), fill=INK)
d.text((plus_x + 76, y + 104), "за минуту разговора.", font=body(28), fill=DIM)
d.text((plus_x + 76, y + 140), "Платите только за принятые", font=body(28), fill=DIM)

d.text((M + 44, y + 182), "Первые 10 заявок — бесплатно, без договора и предоплаты",
       font=semi(28), fill=TEAL_D)
y += box_h + 40
