...
async function findFlats(q, limit) {
  ...
  // объявлении. Район и площадь совпадают у десятков домов в городе, поэтому
  // их мало — дом должен быть подтверждён хоть чем-то, что принадлежит именно
  // ему: координатами, слагом улицы от самой Крыши, точным именем улицы или
  // связкой «ЖК + год постройки», причём и то и другое известны и совпадают,
  // а не просто не мешают. Проверялось на живых данных: без этого правила
  // «пустое поле» само становилось подтверждением, и через него проходили
  // квартиры из разных домов с одинаковым годом «неизвестно».
