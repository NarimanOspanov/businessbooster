...
const INTEGRATION = [
  { key: "ical", label: "Ссылки на календари (iCal)", multiline: true, max: 2000,
    placeholder: "Студия на Абая — https://...ics",
    hint: "По строке на объект: название, тире, ссылка. Такую ссылку отдают Airbnb, " +
      "Booking, RealtyCalendar, Bnovo и Google Календарь — занятость подтянется сама" },
...
function buildRentalPrompt(raw, withTools) {
  ...
  s += "КАК ЗАКРЫВАТЬ БРОНЬ\n" +
    ...
    "предоплату, если она указана ниже. Реквизиты отправь инструментом " +
    "send_payment и попроси " +
    "прислать чек в WhatsApp, туда же, куда пришли фото и реквизиты.\n" +
  ...
  if (withTools) {
    s += "КАЛЕНДАРЬ\n" +
      ...
      "хозяин.\n" +
      "Реквизиты для предоплаты отправляй инструментом send_payment. Номер " +
      "счёта вслух НЕ диктуй — на слух его записывают с ошибкой, и перевод " +
      "уходит чужому человеку. Скажи: отправил реквизиты вам в WhatsApp.\n\n";
  }
...
function rentalToolDefs(baseUrl, toolKey) {
  ...
  base.push({
    ...
    name: "send_payment",
    description:
      "Отправить гостю реквизиты для предоплаты в WhatsApp. Номер счёта голосом " +
      "НЕ диктуй: на слух его записывают с ошибкой, и деньги уходят чужому " +
      "человеку. Вызови инструмент и скажи, что отправил реквизиты в WhatsApp.",
    api_schema: {
      url: baseUrl + "/api/tools/send-payment" + q,
      method: "POST",
      request_body_schema: {
        type: "object",
        properties: {
          phone: {
            type: "string",
            description: "Номер в международном формате. Обычно номер звонящего.",
          },
        },
        required: [],
      },
