...
// @version      2.4
// @description  Берёт из очереди следующий объект без номера, сама жмёт «показать телефон», сохраняет номер и едет дальше сама; снятые и зависшие страницы отмечает промахом с причиной
...
(function () {
  ...
  // Всегда едем сами, без исключения для капчи: она нужна лишь затем, чтобы
  // не объявить страницу мёртвой, пока человек её решает (см. giveUpTimer) —
  // на решение «ехать дальше самому» больше не влияет. Пауза 3 секунды,
  // чтобы сообщение успело мелькнуть на экране.
  function autoNext() {
    nextItem()
      .then(function (q) {
