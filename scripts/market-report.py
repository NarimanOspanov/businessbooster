...
S.append(Paragraph("<b>Справочник видит примерно треть рынка — это измерено.</b> "
                   "По официальным данным в Казахстане работает 6 418 стоматологических "
                   "организаций, 85% из них частные. Справочник, по которому построена "
                   "эта таблица, показывает по стране 2 173 стоматологии — 34% от "
                   "реального числа. Значит все числа в таблице надо умножать примерно "
                   "на три, чтобы получить порядок реальности: 693 стоматологии Алматы — "
                   "это скорее две тысячи, 41 стоматология Костаная — скорее сто двадцать.",
                   styles["p"]))
S.append(Paragraph("Множитель применять к каждому городу по отдельности я не стал: он "
                   "выведен по стране, а покрытие справочника по городам разное. "
                   "Таблицу правильнее читать как нижнюю границу с известным запасом "
                   "втрое.", styles["p"]))
S.append(Paragraph("По Костанаю мы "
